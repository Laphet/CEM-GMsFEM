...
class ProblemSetting(ST.Setting):
    ...
    def set_Robin_func(self,
                       Robin_rhs_func_lf,
                       Robin_rhs_func_rg,
                       Robin_rhs_func_dw,
                       Robin_rhs_func_up,
                       Robin_coeff_func_lf=None,
                       Robin_coeff_func_rg=None,
                       Robin_coeff_func_dw=None,
                       Robin_coeff_func_up=None):
    ...
    def get_coarse_ngh_elem_lim(self, coarse_elem_ind):
        coarse_elem_ind_y, coarse_elem_ind_x = divmod(coarse_elem_ind,
                                                      self.coarse_grid)
        coarse_ngh_elem_lf_lim = max(0,
                                     coarse_elem_ind_x - self.oversamp_layer)
        coarse_ngh_elem_rg_lim = min(
            self.coarse_grid, coarse_elem_ind_x + self.oversamp_layer + 1)
        coarse_ngh_elem_dw_lim = max(0,
                                     coarse_elem_ind_y - self.oversamp_layer)
    ...
    def get_Robin_op_quad_Lag(self, fine_elem_ind_x, fine_elem_ind_y,
                              loc_ind_i, loc_ind_j):
        val = 0.0
        h = self.h
        if fine_elem_ind_x == 0 and loc_ind_i in [0, 2
                                                 ] and loc_ind_j in [0, 2]:
            for quad_ind in range(ST.QUAD_ORDER):
                quad_cord = ST.QUAD_CORD[quad_ind]
                quad_wght = ST.QUAD_WGHT[quad_ind]
                assert hasattr(
                    self, 'Robin_coeff') or self.Robin_coeff_func_lf != None
                if hasattr(self, 'Robin_coeff'):
                    loc_Robin_coeff = self.Robin_coeff[fine_elem_ind_x,
                                                       fine_elem_ind_y]
                if self.Robin_coeff_func_lf != None:
                    center_y = 0.5 * h * (2 * fine_elem_ind_y + 1)
                    quad_real_cord_y = center_y + 0.5 * h * quad_cord
                    loc_Robin_coeff = self.Robin_coeff_func_lf(
                        quad_real_cord_y)
        ...
        if fine_elem_ind_x == self.fine_grid - 1 and loc_ind_i in [
                1, 3
        ] and loc_ind_j in [1, 3]:
            for quad_ind in range(ST.QUAD_ORDER):
                quad_cord = ST.QUAD_CORD[quad_ind]
                quad_wght = ST.QUAD_WGHT[quad_ind]
                assert hasattr(
                    self, 'Robin_coeff') or self.Robin_coeff_func_rg != None
                if hasattr(self, 'Robin_coeff'):
                    loc_Robin_coeff = self.Robin_coeff[fine_elem_ind_x,
                                                       fine_elem_ind_y]
                if self.Robin_coeff_func_rg != None:
                    center_y = 0.5 * h * (2 * fine_elem_ind_y + 1)
                    quad_real_cord_y = center_y + 0.5 * h * quad_cord
                    loc_Robin_coeff = self.Robin_coeff_func_rg(
                        quad_real_cord_y)
                val_i = ST.get_locbase_val(loc_ind_i, 1.0, quad_cord)
                val_j = ST.get_locbase_val(loc_ind_j, 1.0, quad_cord)
                val += 0.5 * h * quad_wght * loc_Robin_coeff * val_i * val_j
        if fine_elem_ind_y == 0 and loc_ind_i in [0, 1
                                                 ] and loc_ind_j in [0, 1]:
            for quad_ind in range(ST.QUAD_ORDER):
                quad_cord = ST.QUAD_CORD[quad_ind]
                quad_wght = ST.QUAD_WGHT[quad_ind]
                assert hasattr(
                    self, 'Robin_coeff') or self.Robin_coeff_func_dw != None
                if hasattr(self, 'Robin_coeff'):
                    loc_Robin_coeff = self.Robin_coeff[fine_elem_ind_x,
                                                       fine_elem_ind_y]
                ...
                val += 0.5 * h * quad_wght * loc_Robin_coeff * val_i * val_j
        if fine_elem_ind_y == self.fine_grid - 1 and loc_ind_i in [
                2, 3
        ] and loc_ind_j in [2, 3]:
            for quad_ind in range(ST.QUAD_ORDER):
                quad_cord = ST.QUAD_CORD[quad_ind]
                quad_wght = ST.QUAD_WGHT[quad_ind]
                assert hasattr(
                    self, 'Robin_coeff') or self.Robin_coeff_func_up != None
                if hasattr(self, 'Robin_coeff'):
                    loc_Robin_coeff = self.Robin_coeff[fine_elem_ind_x,
                                                       fine_elem_ind_y]
                if self.Robin_coeff_func_up != None:
                    center_x = 0.5 * h * (2 * fine_elem_ind_x + 1)
                    quad_real_cord_x = center_x + 0.5 * h * quad_cord
                    loc_Robin_coeff = self.Robin_coeff_func_up(
                        quad_real_cord_x)
                val_i = ST.get_locbase_val(loc_ind_i, quad_cord, 1.0)
                val_j = ST.get_locbase_val(loc_ind_j, quad_cord, 1.0)
                val += 0.5 * h * quad_wght * loc_Robin_coeff * val_i * val_j
    ...
    def get_Robin_rhs_quad_Lag(self, fine_elem_ind_x, fine_elem_ind_y,
                               loc_ind):
        val = 0.0
        h = self.h
    ...
    def get_source_quad_Lag(self, fine_elem_ind_x, fine_elem_ind_y, loc_ind):
        val = 0.0
        h = self.h
        center_x, center_y = 0.5 * h * (2 * fine_elem_ind_x +
                                        1), 0.5 * h * (2 * fine_elem_ind_y + 1)
        for quad_ind_x in range(ST.QUAD_ORDER):
            for quad_ind_y in range(ST.QUAD_ORDER):
                quad_cord_x, quad_cord_y = ST.QUAD_CORD[
                    quad_ind_x], ST.QUAD_CORD[quad_ind_y]
                quad_wght_x, quad_wght_y = ST.QUAD_WGHT[
                    quad_ind_x], ST.QUAD_WGHT[quad_ind_y]
                quad_real_cord_x = center_x + 0.5 * h * quad_cord_x
                quad_real_cord_y = center_y + 0.5 * h * quad_cord_y
                test_val = ST.get_locbase_val(loc_ind, quad_cord_x,
                                              quad_cord_y)
    ...
    def get_eigen_pair(self):
        ...
        S_mat_list = [
            None
        ] * self.coarse_elem  # A list of S matrices, saved here for futural usages
        A_mat_list = [None] * self.coarse_elem
        for coarse_elem_ind in range(self.coarse_elem):
            coarse_elem_ind_y, coarse_elem_ind_x = divmod(
                coarse_elem_ind, self.coarse_grid)
            ...
            for sub_elem_ind in range(self.sub_elem):
                sub_elem_ind_y, sub_elem_ind_x = divmod(
                    sub_elem_ind, self.sub_grid)
                ...
                for loc_ind_i in range(ST.N_V):  # i=0,1,2,3
                    for loc_ind_j in range(ST.N_V):  # j=0,1,2,3
                        ...
                        I[marker] = (sub_elem_ind_y + iy) * (
                            self.sub_grid + 1) + sub_elem_ind_x + ix
                        jy, jx = divmod(loc_ind_j, 2)
                        # J_loc[temp_ind] = (sub_elem_ind_y+jy) * (self.sub_grid+1) + sub_elem_ind_x + jx
                        J[marker] = (sub_elem_ind_y + jy) * (
                            self.sub_grid + 1) + sub_elem_ind_x + jx
                        ...
                        A_val[marker] = loc_coeff * (
                            self.elem_Lap_stiff_mat[loc_ind_i, loc_ind_j])
                        A_val[marker] += self.get_Robin_op_quad_Lag(
                            fine_elem_ind_x, fine_elem_ind_y, loc_ind_i,
                            loc_ind_j)
                        S_val[marker] = loc_kappa * (
                            self.elem_Bi_mass_mat[loc_ind_i, loc_ind_j])
                        marker += 1
            ...
            A_mat_coo = coo_matrix(
                (A_val[:marker], (I[:marker], J[:marker])),
                shape=(fd_num,
                       fd_num))  # A nicer method of constructing FEM matrices
            S_mat_coo = coo_matrix(
                (S_val[:marker], (I[:marker], J[:marker])),
                shape=(fd_num,
                       fd_num))  # A nicer method of constructing FEM matrices
            ...
            val, vec = eigsh(A_mat,
                             ...
                             M=S_mat,
                             sigma=-1.0,
                             which='LM')  # Refer Scipy documents
            ...
            eigen_val[coarse_elem_ind * self.eigen_num:(coarse_elem_ind + 1) *
                      self.eigen_num] = val
            eigen_vec[:,
                      coarse_elem_ind * self.eigen_num:(coarse_elem_ind + 1) *
                      self.eigen_num] = vec
    ...
    def get_corr_basis(self):
        ...
        max_data_len = (2 * self.oversamp_layer + 1)**2 * (
            (self.sub_grid + 1)**4 + self.sub_elem * ST.N_V**2)
        prc_flag = 1
        for coarse_elem_ind in range(self.coarse_elem):
            ...
            for coarse_ngh_elem_ind_y in range(dw_lim, up_lim):
                for coarse_ngh_elem_ind_x in range(lf_lim, rg_lim):
                    ...
                    eigen_vec = self.eigen_vec[:, coarse_ngh_elem_ind *
                                               self.eigen_num:
                                               (coarse_ngh_elem_ind + 1) *
                                               self.eigen_num]
                    ...
                    for node_sub_ind in range((self.sub_grid + 1)**2):
                        node_sub_ind_y, node_sub_ind_x = divmod(
                            node_sub_ind, self.sub_grid + 1)
                        ...
                        node_ind = node_ind_y * (self.fine_grid +
                                                 1) + node_ind_x
                    ...
                    for ind_i in range(marker_):
                        ...
                        if coarse_ngh_elem_ind == coarse_elem_ind:
                            for eigen_ind in range(self.eigen_num):
                                rhs_basis[fd_ind_i,
                                          eigen_ind] += P_mat[node_sub_ind_i,
                                                              eigen_ind]
                                guess[fd_ind_i,
                                      eigen_ind] += eigen_vec[node_sub_ind_i,
                                                              eigen_ind]
                    for sub_elem_ind in range(self.sub_elem):
                        sub_elem_ind_y, sub_elem_ind_x = divmod(
                            sub_elem_ind, self.sub_grid)
                        ...
                        loc_coeff = self.coeff[fine_elem_ind_x,
                                               fine_elem_ind_y]
                        for loc_ind_i in range(ST.N_V):
                            loc_ind_iy, loc_ind_ix = divmod(loc_ind_i, 2)
                            node_ind_i = (fine_elem_ind_y + loc_ind_iy) * (
                                self.fine_grid +
                                1) + fine_elem_ind_x + loc_ind_ix
                            if node_ind_i in ind_map_dic:
                                fd_ind_i = ind_map_dic[node_ind_i]
                                for loc_ind_j in range(ST.N_V):
                                    loc_ind_jy, loc_ind_jx = divmod(
                                        loc_ind_j, 2)
                                    node_ind_j = (
                                        fine_elem_ind_y + loc_ind_jy) * (
                                            self.fine_grid +
                                            1) + fine_elem_ind_x + loc_ind_jx
                                    if node_ind_j in ind_map_dic:
                                        fd_ind_j = ind_map_dic[node_ind_j]
                                        I[marker] = fd_ind_i
                                        J[marker] = fd_ind_j
                                        V[marker] = loc_coeff * (
                                            self.elem_Lap_stiff_mat[loc_ind_i,
                                                                    loc_ind_j])
                                        V[marker] += self.get_Robin_op_quad_Lag(
                                            fine_elem_ind_x, fine_elem_ind_y,
                                            loc_ind_i, loc_ind_j)
                                        marker += 1
                                if coarse_ngh_elem_ind == coarse_elem_ind:
                                    rhs_corr[
                                        fd_ind_i] += self.get_Robin_rhs_quad_Lag(
                                            fine_elem_ind_x, fine_elem_ind_y,
                                            loc_ind_i)
            Op_mat_coo = coo_matrix((V[:marker], (I[:marker], J[:marker])),
                                    shape=(fd_num, fd_num))
            ...
            if coarse_elem_ind > prc_flag / 10 * self.coarse_elem:
                logging.info("......{0:.2f}%".format(coarse_elem_ind /
                                                     self.coarse_elem * 100.))
                prc_flag += 1
    ...
    def get_glb_A_F(self):
        ...
        for fine_elem_ind in range(self.fine_elem):
            fine_elem_ind_y, fine_elem_ind_x = divmod(fine_elem_ind,
                                                      self.fine_grid)
            ...
            for loc_ind_i in range(ST.N_V):
                loc_ind_iy, loc_ind_ix = divmod(loc_ind_i, 2)
                node_ind_i = (fine_elem_ind_y + loc_ind_iy) * (
                    self.fine_grid + 1) + fine_elem_ind_x + loc_ind_ix
                for loc_ind_j in range(ST.N_V):
                    loc_ind_jy, loc_ind_jx = divmod(loc_ind_j, 2)
                    node_ind_j = (fine_elem_ind_y + loc_ind_jy) * (
                        self.fine_grid + 1) + fine_elem_ind_x + loc_ind_jx
                    I[marker] = node_ind_i
                    J[marker] = node_ind_j
                    V[marker] = elem_stiff_mat[loc_ind_i, loc_ind_j]
                    V[marker] += self.get_Robin_op_quad_Lag(
                        fine_elem_ind_x, fine_elem_ind_y, loc_ind_i, loc_ind_j)
                    marker += 1
                glb_F_vec[node_ind_i] += self.get_source_quad_Lag(
                    fine_elem_ind_x, fine_elem_ind_y, loc_ind_i)
                glb_F_vec[node_ind_i] += self.get_Robin_rhs_quad_Lag(
                    fine_elem_ind_x, fine_elem_ind_y, loc_ind_i)
        glb_A_mat_coo = coo_matrix((V[:marker], (I[:marker], J[:marker])),
                                   shape=(tot_node, tot_node))
        ...
        for coarse_elem_ind in range(self.coarse_elem):
            for eigen_ind in range(self.eigen_num):
                ...
                for loc_fd_ind, node_ind in self.ind_map_rev_list[
                        coarse_elem_ind].items():
                    I[marker] = node_ind
                    J[marker] = fd_ind
                    V[marker] = self.basis_list[coarse_elem_ind][loc_fd_ind,
                                                                 eigen_ind]
                    marker += 1
        glb_basis_spmat = csc_matrix((V[:marker], (I[:marker], J[:marker])),
                                     shape=(self.tot_node, self.tot_fd_num))
        glb_basis_spmat_T = csc_matrix((V[:marker], (J[:marker], I[:marker])),
    ...
    def solve(self, guess=[]):
        ...
        self.get_corr_basis()
        logging.info(
            "Finish getting the Neumann corrector and multiscale bases.")
        self.get_glb_A_F()
        logging.info(
            "Finish getting the global stiffness matrix and right-hand vector."
        )
        ...
        rhs = self.glb_basis_spmat_T.dot(self.glb_F_vec -
                                         self.glb_A_mat.dot(self.glb_corr))
    ...
    def solve_ref(self, guess=[]):
        ...
        def get_fd_ind(fine_elem_ind_x, fine_elem_ind_y, loc_ind):
            ...
            return (fine_elem_ind_y + loc_ind_y) * (
                self.fine_grid + 1) + fine_elem_ind_x + loc_ind_x
        ...
        for fine_elem_ind in range(self.fine_elem):
            ...
            loc_coeff = self.coeff[fine_elem_ind_x, fine_elem_ind_y]
            for loc_ind_i in range(ST.N_V):
                fd_ind_i = get_fd_ind(fine_elem_ind_x, fine_elem_ind_y,
                                      loc_ind_i)
                if fd_ind_i >= 0:
                    for loc_ind_j in range(ST.N_V):
                        fd_ind_j = get_fd_ind(fine_elem_ind_x, fine_elem_ind_y,
                                              loc_ind_j)
                        if fd_ind_j >= 0:
                            I[marker] = fd_ind_i
                            J[marker] = fd_ind_j
                            V[marker] = loc_coeff * self.elem_Lap_stiff_mat[
                                loc_ind_i, loc_ind_j]
                            V[marker] += self.get_Robin_op_quad_Lag(
                                fine_elem_ind_x, fine_elem_ind_y, loc_ind_i,
                                loc_ind_j)
                            marker += 1
                    rhs[fd_ind_i] += self.get_source_quad_Lag(
                        fine_elem_ind_x, fine_elem_ind_y, loc_ind_i)
                    rhs[fd_ind_i] += self.get_Robin_rhs_quad_Lag(
                        fine_elem_ind_x, fine_elem_ind_y, loc_ind_i)
        A_mat_coo = coo_matrix((V[:marker], (I[:marker], J[:marker])),
                               shape=(fd_num, fd_num))
        A_mat = A_mat_coo.tocsc()
        logging.info(
            "Finish constructing the final linear system of the reference problem."
        )
        # ilu = spilu(A_mat)
        # Mx = lambda x: ilu.solve(x)
        # pre_M = LinearOperator((fd_num, fd_num), Mx)
        ...
        # assert info == 0
        u_ref_inner = spsolve(A_mat, rhs)
        u_ref = np.zeros((self.tot_node,))
        u_ref[:fd_num] = u_ref_inner
        logging.info(
            "Finish solving the final linear system of the reference problem.")
        return u_ref
                ...
                for coarse_ngh_elem_ind_y in range(dw_lim, up_lim):
                    ...
                    for node_sub_ind_y in range(self.sub_grid + 1):
                        for node_sub_ind_x in range(self.sub_grid + 1):
                            node_sub_ind = node_sub_ind_y * (
                                self.sub_grid + 1) + node_sub_ind_x
                            node_ind_y = coarse_ngh_elem_ind_y * self.sub_grid + node_sub_ind_y
                            node_ind_x = coarse_ngh_elem_ind_x * self.sub_grid + node_sub_ind_x
                            node_ind = node_ind_y * (self.fine_grid +
                                                     1) + node_ind_x
    ...
    def get_true_corr(self, guess=[]):
        ...
        for coarse_elem_ind in range(self.coarse_elem):
            ...
            eigen_vec = self.eigen_vec[:, coarse_elem_ind *
            ...
            for node_sub_ind_y in range(self.sub_grid + 1):
                for node_sub_ind_x in range(self.sub_grid + 1):
                    node_sub_ind = node_sub_ind_y * (self.sub_grid +
                                                     1) + node_sub_ind_x
                ...
                for loc_ind_i in range(ST.N_V):
                    ...
                    for loc_ind_j in range(ST.N_V):
                        ...
                        V[marker] += self.get_Robin_op_quad_Lag(
                            ...
                            loc_ind_j)
                        marker += 1
                    rhs_corr[fd_ind_i] += self.get_Robin_rhs_quad_Lag(
                        fine_elem_ind_x, fine_elem_ind_y, loc_ind_i)
        Op_mat_coo = coo_matrix((V[:marker], (I[:marker], J[:marker])),
                                shape=(fd_num, fd_num))
        Op_mat = Op_mat_coo.tocsc()
